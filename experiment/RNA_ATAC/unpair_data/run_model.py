...
if data == 'muto':
    ...
    if 'scglue_amplification':
        combine_list = []
        for i in range(5):
            combine = sc.read_h5ad('/home/atac2rna/data/atac2rna/model_output/22_10_12/unpaired_data/scglue/muto/'+str(i+1)+'/embedding.h5ad')
...
if data == 'yao':
    ...
    if 'scglue_integ':
        combine_list = []
        for i in range(4):
            combine = sc.read_h5ad('/home/atac2rna/data/atac2rna/model_output/22_10_12/unpaired_data/scglue/yao/'+str(i+1)+'/embedding.h5ad')
            combine_list.append(combine)
...
if model_type == 'basic':
    if data == 'yao':
        id_list = unpaired_split_dataset_yao(RNA_data, ATAC_data)
    if data == 'muto':
        id_list = unpaired_split_dataset_muto(RNA_data, ATAC_data)

if model_type == 'scglue_integ':
    if data == 'yao':
        id_list = unpaired_split_dataset_yao_scglue(RNA_data, ATAC_data)
    if data == 'muto':
        id_list = unpaired_split_dataset_muto_scglue(RNA_data, ATAC_data)
