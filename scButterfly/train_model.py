...
class Model():
    ...
    def evaluation(
        ...
        sum_aupr = 0
        max_enum = 9
        RNA_input_dim = RNA_data.shape[1]
        ATAC_input_dim = ATAC_data.shape[1]
        ...
        with torch.no_grad():
            with tqdm(total = len(self.test_dataloader), ncols=100) as pbar:
                pbar.set_description('evaluating correlation')
                for idx, batch_samples in enumerate(self.test_dataloader):
                    if torch.cuda.is_available():
                        batch_samples = batch_samples.cuda().to(torch.float32)
        ...
        mean_test_pearson /= len(self.test_dataloader)
        mean_test_spearman /= len(self.test_dataloader)
    ...
    def test(
        ...
        self.A_test_dataloader = DataLoader(self.A_test_dataset, batch_size=64, shuffle=False, num_workers=4)
        ...
        if test_evaluate:
            ...
            [mean_test_auroc,mean_test_aupr,mean_test_pearson,mean_test_spearman] = self.evaluation(10, self.RNA_data, self.ATAC_data, test_id_r)
            
            with open(output_path + '/evaluation.txt','w') as f:  
                f.write('*****mean_test_pearson: ' + str(mean_test_pearson) + '*****')
                f.write('*****mean_test_spearman: ' + str(mean_test_spearman) + '*****')
                f.write('*****mean_test_auroc: ' + str(mean_test_auroc) + '*****')
                f.write('*****mean_test_aupr: ' + str(mean_test_aupr) + '*****')
